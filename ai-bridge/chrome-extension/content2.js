...
function extractMarkdown(element) {
  ...
  for (const node of element.childNodes) {
    ...
    switch (tag) {
      ...
      case "a":
        if (node.closest(".ds-message") && !node.textContent.trim()) break;
  ...
  try {
    switch (action) {
      case "new_session": {
        ...
        t("new_session: 已发送，后台轮询中...");
        injectBridgeCheck(config);
        return { status: "sent", sessionUrl: window.location.href };
      }
      case "send_message": {
        t("send_message: 准备输入");
        const prevCount = countResponses(config);
